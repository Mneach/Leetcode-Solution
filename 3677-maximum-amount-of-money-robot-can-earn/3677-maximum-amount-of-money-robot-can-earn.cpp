...
public:
    ...
    int dfs(vector<vector<int>>& coins, vector<vector<vector<int>>> &answer, int row, int col, int k) {
        ...
        if (row < 0 || row >= totalRow) return INITIAL_VALUE;
        else if(col < 0 || col >= totalCol) return INITIAL_VALUE;
        else if (answer[row][col][k] != INITIAL_VALUE) return answer[row][col][k];

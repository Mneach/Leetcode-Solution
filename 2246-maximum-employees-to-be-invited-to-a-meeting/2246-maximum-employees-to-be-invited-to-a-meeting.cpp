/*
Intuition : 
1. if the cyclic in 1 group is more than 2 nodes, that result should consider as an answer
...
2. if the cyclic in 1 group is between 2 nodes, the result should be total node in that group
favorite = [1,2,3,4,5,4]
answer = 6
cyclic result = 0 -> 1 -> 2 -> 3 -> 4 <-> 5

3. we alwasy can combine the group that have a cyclic in between 2 node, so this should consider as the result, since we can choose multiple group with this condition.

4. if there are more than 1 group in favorit answer, we just need to compare those groups using the rules above
    ...
    int maximumInvitations(vector<int>& favorite) {
        ...
        for (int i = 0; i < favorite.size(); i++) {
            ...
            while(visited[current] == false) {
                ...
                if (length == 2) {
                    // cycle between node is 2, that means we can add more node in here
        ...
        for (auto data : cycle2) {
            ...
            secondAnswer += bfs(firstNode, secondNode, invertedGraph) + bfs(secondNode, firstNode, invertedGraph) + 2;
        }
